...
export async function GET(_request: NextRequest, context: RouteContext) {
  try {
    ...
    const response = await searchFetch(`/knowledgebases/${encodeURIComponent(params.id)}`)
...
export async function DELETE(_request: NextRequest, context: RouteContext) {
  try {
    const params = context.params instanceof Promise ? await context.params : context.params
    const response = await searchFetch(`/knowledgebases/${encodeURIComponent(params.id)}`, { method: 'DELETE' })
    const text = await response.text()
    const data = text ? JSON.parse(text) : { success: response.ok }
    return NextResponse.json(data, { status: response.status })
  } catch (error) {
