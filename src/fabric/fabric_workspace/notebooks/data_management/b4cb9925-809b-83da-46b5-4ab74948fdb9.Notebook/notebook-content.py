...
print("📋 Schemas to be truncated: customer, product, sales, finance, inventory, supplychain, salescrm, shared")
...
product_tables = ['Product', 'ProductCategory', 'ProductLine']
...
supplychain_tables = ['Suppliers', 'ProductSuppliers', 'SupplyChainEvents', 'SupplyChainEventImpacts']
...
# ### Truncate Sales CRM Schema Tables

# CELL ********************

print("🤝 Truncating Sales CRM schema tables...")
...
for table in salescrm_tables:
    try:
        spark.sql(f"TRUNCATE TABLE salescrm.{table}")
        print(f"   ✅ salescrm.{table} truncated")
    except Exception as e:
        print(f"   ❌ Error truncating salescrm.{table}: {str(e)}")

print("✅ Sales CRM schema truncation complete!")
...
print("📅 Truncating Shared schema tables...")

# Shared schema tables
shared_tables = ['DimDate']

for table in shared_tables:
    try:
        spark.sql(f"TRUNCATE TABLE shared.{table}")
        print(f"   ✅ shared.{table} truncated")
    except Exception as e:
        print(f"   ❌ Error truncating shared.{table}: {str(e)}")

print("✅ Shared schema truncation complete!")
...
print("   • Product: 3 tables truncated")
