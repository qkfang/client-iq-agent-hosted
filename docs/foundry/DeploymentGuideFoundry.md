...
- **`gpt-5.6-sol`** (or your `AZURE_CHAT_MODEL` override) available in the target `AZURE_AI_DEPLOYMENTS_LOCATION` region.
...
| **Azure OpenAI deployments** | `gpt-5.6-sol` for chat (150K TPM) and `text-embedding-3-small` for embeddings (80K TPM). |
...
| Setting | Value |
|---|---|
| **Model** | `gpt-5.6-sol` (override with `AZURE_CHAT_MODEL` or `AZURE_AI_AGENT_MODEL_DEPLOYMENT_NAME`) |
...
| `AZURE_CHAT_MODEL` | Chat completion model name | `gpt-5.6-sol` |
...
**Why**: The target region may not have sufficient quota for `gpt-5.6-sol` (150K TPM) or `text-embedding-3-small` (80K TPM).
