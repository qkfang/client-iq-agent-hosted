...
| | `AZURE_OPENAI_DEPLOYMENT_MODEL` | GPT model to deploy | `gpt-5.6-sol` | `azd env set AZURE_OPENAI_DEPLOYMENT_MODEL gpt-4o` |
...
- **[Azure OpenAI Models](https://learn.microsoft.com/azure/ai-services/openai/)**:
  - [`gpt-5.6-sol`](https://learn.microsoft.com/azure/ai-services/openai/concepts/models) - Chat completion (150K TPM)
...
| **Azure OpenAI deployments** | Two model deployments inside the Foundry project: a chat completion model (default `gpt-5.6-sol`) and an embedding model (default `text-embedding-3-small`). |
...
2. **Agents** → agents named `ChatAgent` and `OnboardingAgent` exist, their model matches `AZURE_AI_AGENT_MODEL_DEPLOYMENT_NAME` / `AZURE_CHAT_MODEL` (default `gpt-5.6-sol`), and the **Tools** panel shows the `{solution_suffix}-kb-mcp-connection` MCP tool attached.
